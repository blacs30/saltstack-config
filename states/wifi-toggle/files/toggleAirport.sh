...
activate
display dialog "Wifi / Cable Network status probably changed if you see this." with title "Wifi Toggle" buttons {"OK"} default button 1 giving up after 30
end tell
EOF
    fi
...
eth_names=`networksetup -listnetworkserviceorder | sed -En 's|^\(Hardware Port: .*(Slot 1\|Slot 2\|Ethernet\|LAN), Device: (en[0123456789]+)\)$|\2|p'`
